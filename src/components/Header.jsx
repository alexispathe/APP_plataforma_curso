                  ...
                  <ul className="header-section">
                    {
                      ...
                      sections.map((section, i) => (
                        ...
                        >
                          <Link
                            to={"/seccion/" + section.sectionURL}
                            className="header-section-item-a"
                          >
                            {section.name} <IoIosArrowDown />
                          </Link>
                          <ul className="h-s-sub-item-container">
                            {categories && categories.length >= 1 ? ( //Aqui estamos devolviendo todas categorias de BD para luego compararlos mediante su 'sectionID' y hacer una relacion
                              categories.map((category, j) =>
                                category.sectionID === section.sectionID ? (
                                  <li
                                    className="h-s-sub-item"
                                    key={category.categoryID || j}
                                  >
                                    <Link to={"/seccion/" +section.sectionURL +"/" +category.categoryURL}>
                                      {category.name}
                                    </Link>
                                  </li>
                                ) : (
                                  <p className="text-white">
                                    ¡Estamos trabajando para traer contenido!
                                  </p>
                                )
                              )
                            ) : (
